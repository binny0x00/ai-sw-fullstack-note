...
- AI 검토 자동 준비는 NestJS 프로세스 내부의 best-effort 백그라운드 실행이므로 서버 재시작 시 진행 중 작업이 유실될 수 있다.
- 게시글 댓글까지 RAG VectorStore에 실시간 적재하는 흐름은 아직 없다.
- GitHub Issue 생성은 외부 시스템 변경이므로 자동 실행하지 않고 담당자 승인 후에만 실행한다.

## 12. 개선 아이디어

- BullMQ, Redis 같은 작업 큐로 AI 검토 작업을 분리
- 댓글 변경 시 게시글과 댓글 대화를 RAG VectorStore에 재적재
